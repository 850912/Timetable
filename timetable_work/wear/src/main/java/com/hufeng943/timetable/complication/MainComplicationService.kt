...
class MainComplicationService : SuspendingTimelineComplicationDataSourceService() {
    ...
    override suspend fun onComplicationRequest(request: ComplicationRequest): ComplicationDataTimeline? {
        ...
        for (targetDate in daysToScan) {
            for (occurrence in NextCourseEngine.occurrencesForDate(allTimetables, targetDate, timeZone)) {
                if (occurrence.endInstant > todayStartInstant && occurrence.startInstant < timelineEndInstant) {
                    activeIntervals.add(
                        CourseInterval(
                            name = occurrence.courseName,

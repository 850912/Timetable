...
class WearOsTransport(
    ...
    override suspend fun send(timetables: List<Timetable>): SyncResult = SyncResult.Failed("已切换到增量同步，请先产生同步变更")
    ...
    override suspend fun sendRecords(records: List<SyncRecordPayload>): SyncResult {
        return try {
            ...
            val requestId = System.currentTimeMillis().toString() + "-" + records.size

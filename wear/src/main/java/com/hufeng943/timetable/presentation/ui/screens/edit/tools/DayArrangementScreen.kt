...
fun DayArrangementScreen(viewModel: ScheduleAdjustmentViewModel = hiltViewModel()) {
    ...
    when(val current=state){
        ...
        is ScheduleAdjustmentState.Ready -> {
            val table=current.timetables.first()
            ...
            SwipeDismissableNavHost(navController=nav,startDestination=DayRoutes.MAIN){
                composable(DayRoutes.MAIN){
                    ...
                    ScreenScaffold(scrollState=scroll,timeText={},edgeButton={EdgeButton(enabled=sourceDay!=date.dayOfWeek,onClick={viewModel.applyDayArrangement(table.timetableId,date,sourceDay)}){Icon(Icons.Rounded.Check,"确认")}}){padding->
                        TransformingLazyColumn(state=scroll,contentPadding=padding,modifier=Modifier.fillMaxSize()){
                            ...
                            item{OneUiCapsuleSurface(title="恢复调休/换课",subtitle="恢复由调休、课程调节产生的修改",icon=Icons.Rounded.Restore,onClick={viewModel.restoreAdjustments(table.timetableId)},modifier=Modifier.fillMaxWidth().transformedHeight(this,transform).minimumVerticalContentPadding(ButtonDefaults.minimumVerticalListContentPadding))}
                        }
                    }
                }

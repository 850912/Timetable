        ...
        composable("main") {
            ...
            ScreenScaffold(scrollState = state) { padding ->
                TransformingLazyColumn(state = state, rotaryScrollableBehavior = RotaryScrollableDefaults.snapBehavior(state), modifier = Modifier.fillMaxSize(), contentPadding = padding) {
                    ...
                    item { OneUiCapsuleSurface(title="镜头畸变",subtitle="${(config.glassLensDistortion*100).toInt()}% · 0% 可关闭折射",icon=Icons.Rounded.Tune,onClick={openAdjust(AdjustTarget("镜头畸变",(config.glassLensDistortion*100).toInt(),0..100,"%") { onLensDistortionChange(it/100f) })},modifier=itemModifier(this)) }
                    ...
                    if(config.glassBlurEnabled)item { OneUiCapsuleSurface(title="玻璃模糊强度",subtitle="${config.glassBlurRadius.toInt()} dp · 点按精确调节",icon=Icons.Rounded.BlurOn,onClick={openAdjust(AdjustTarget("玻璃模糊强度",config.glassBlurRadius.toInt(),0..8," dp") { onBlurRadiusChange(it.toFloat()) })},modifier=itemModifier(this)) }

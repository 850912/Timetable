...
fun SettingScreen() {
    ...
    SettingPager(
        ...
        onReminderSelectClick = { nav.navigateSingle(NavRoutes.MORE_SETTINGS_REMINDER) },
...
@Composable
fun SettingsReminderScreen(appConfigViewModel: AppConfigViewModel) {
    val nav = LocalNavController.current
    ReminderSelectPager(LocalAppConfig.current) { minutes ->
        appConfigViewModel.updateCourseReminderMinutes(minutes)
        nav.popSafe()
    }
}

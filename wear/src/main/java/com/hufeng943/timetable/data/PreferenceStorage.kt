...
class PreferenceStorage @Inject constructor(
    ...
    val appConfigFlow: Flow<AppConfig> = context.dataStore.data.map { prefs ->
        ...
        AppConfig(
            ...
            courseReminderMinutes = prefs[Keys.COURSE_REMINDER_MINUTES]?.toIntOrNull()?.takeIf { it in setOf(5, 15, 30) },
        )
    }
    ...
    suspend fun setCourseReminderMinutes(minutes: Int?) {
        require(minutes == null || minutes in setOf(5, 15, 30)) { "Unsupported reminder offset" }
        context.dataStore.edit { prefs ->
            if (minutes == null) prefs.remove(Keys.COURSE_REMINDER_MINUTES)
            else prefs[Keys.COURSE_REMINDER_MINUTES] = minutes.toString()
        }
    }
}
